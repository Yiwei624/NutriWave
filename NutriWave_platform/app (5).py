...
from core.engine import UserRequest, generate_candidates, resolve_structure_kpi
...
def _render_process_window_card(candidate):
    """Render the factory-facing process window below the JSON audit trail."""
    ...
    with st.container(border=True):
        st.markdown(f"##### {t('process_window_title')}  ·  `{pwin.get('window_id', 'PW-v1')}`")
        m1, m2, m3 = st.columns(3)
        m1.metric(t("fermentation_temp_range"), temp_display)
        m2.metric(t("max_shear_rpm"), rpm_display)
        m3.metric(
            t("qc_stop_gate"),
            f"pH {pH_gate.get('operator', '<=')} {pH_gate.get('target', 4.6)} + η {visc_gate.get('operator', '>=')} {visc_gate.get('target', '—')} Pa·s",
        )

        s1, s2, s3 = st.columns(3)
        s1.metric(t("predicted_yield_stress"), f"{physical.get('yield_stress_Pa', '—')} Pa")
        s2.metric(t("predicted_viscosity"), f"{physical.get('rheological_viscosity_Pa_s', '—')} Pa·s")
        s3.metric(t("structure_release_gate"), release_display)

        st.success(f"QC：{stop_display}" if lang == "zh" else f"QC: {stop_display}")
        st.caption(
            f"{t('scaleup_note')}: "
