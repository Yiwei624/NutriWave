...
# ---------- Recipe Engine (MINIMAL + Consumer Data) ----------
    ...
    if generate:
        # infer goals from brief and texture (v1, rule-based)
        ...
        # adjust goals based on customer_profile (v1, small bias)
        def apply_customer_profile(goals: list, profile: dict | None) -> list:
            ...
            # assumes 1-5 scale; adjust gently
            if bm is not None and bm < 3.0:
                # force anti_beany to front
...
# ---------- Admin-only Database ----------
    ...
    st.caption("Admin-only. Seed library only (no customer-facing logging).")
...
st.sidebar.markdown("---")
st.sidebar.info("NutriWave | Minimal Engine (3 inputs) + Consumer Data Import + Admin DB")
