...
def simplify_candidate(candidate: Dict[str, Any], lang: str = "zh") -> Dict[str, Any]:
    """Compact JSON for humans.

    The old payload was useful for debugging but too dense for customers. This
    returns the production-facing representation: structure target, process
    window, QC gates, predicted physical KPIs, and a simple formulation table.
    """
    pwin = candidate.get("process_window", {}) or {}
    physical = candidate.get("predicted_physical_kpis", {}) or {}
    structure = candidate.get("structure_kpi", {}) or {}
    display = (pwin.get("display", {}) or {}).get(lang, {}) or {}
    formulation = candidate.get("formulation", {}) or {}
    ingredients = []
    for it in (formulation.get("ingredients", []) or []):
        ingredients.append({
            "ingredient": it.get("ingredient_id", "TBD"),
            "kg_per_100kg": it.get("dosage_kg", 0),
        })
    ...
    return {
        ...
        "deliverable": "process_window_not_recipe",
        "strain_combo_id": candidate.get("strain_combo_id"),
        "status": physical.get("status", "review"),
        "structure_targets": {
            "yield_stress": ((structure.get("yield_stress", {}) or {}).get("display") or "> 25 Pa"),
            "viscosity": ((structure.get("viscosity", {}) or {}).get("display") or "> 1.5 Pa·s"),
            "syneresis": ((structure.get("syneresis", {}) or {}).get("display") or "≤ 6%"),
        },
        "process_window": {
            "fermentation_temperature_C": display.get(
                "fermentation_temperature_C",
                (pwin.get("fermentation_temperature_C", {}) or {}).get("display", "—"),
            ),
            "maximum_shear_rpm": display.get(
                "max_shear_rpm",
                (pwin.get("maximum_shear", {}) or {}).get("display", "—"),
            ),
            "stop_condition": display.get("qc_stop_condition", "pH <= 4.6 and viscosity gate reached"),
        },
        "qc_gates": {
            "pH_end": (stop.get("pH_end", {}) or {}).get("target", 4.6),
            "viscosity_min_Pa_s": (stop.get("rheological_viscosity_Pa_s", {}) or {}).get("target", 1.5),
            "yield_stress_min_Pa": (release.get("yield_stress_Pa", {}) or {}).get("target", 25),
            "syneresis_max_pct": (release.get("syneresis_pct", {}) or {}).get("target", 6),
        },
        "predicted_physical_kpis": {
            "yield_stress_Pa": physical.get("yield_stress_Pa"),
            "viscosity_Pa_s": physical.get("rheological_viscosity_Pa_s"),
            "syneresis_max_pct": physical.get("syneresis_pct_max"),
        },
        "formulation_table": ingredients,
    }
...
def evaluate_qc_feedback(candidate: Dict[str, Any], feedback: Dict[str, Any]) -> Dict[str, Any]:
    """Evaluate factory QC feedback against the predicted process window."""
    pwin = candidate.get("process_window", {}) or {}
    qc = pwin.get("qc_gates", {}) or {}
    stop = qc.get("fermentation_stop", {}) or {}
    release = qc.get("structure_release", {}) or {}

    viscosity_target = _as_float((stop.get("rheological_viscosity_Pa_s", {}) or {}).get("target"), 1.5)
    yield_target = _as_float((release.get("yield_stress_Pa", {}) or {}).get("target"), 25.0)
    syneresis_max = _as_float((release.get("syneresis_pct", {}) or {}).get("target"), 6.0)

    ph_4h = _as_float(feedback.get("ph_4h"), 0.0)
    measured_viscosity = _as_float(feedback.get("measured_viscosity_Pa_s"), 0.0)
    measured_yield = feedback.get("measured_yield_stress_Pa")
    measured_yield_f = None if measured_yield in [None, ""] else _as_float(measured_yield, 0.0)
    syneresis_observed = bool(feedback.get("syneresis_observed", False))
    syneresis_pct = feedback.get("syneresis_pct")
    syneresis_pct_f = None if syneresis_pct in [None, ""] else _as_float(syneresis_pct, 0.0)

    fail_reasons: List[str] = []
    warnings: List[str] = []

    if measured_viscosity < viscosity_target:
        fail_reasons.append(
            f"viscosity below target: {measured_viscosity:.2f} < {viscosity_target:.2f} Pa·s"
        )
    if measured_yield_f is not None and measured_yield_f < yield_target:
        fail_reasons.append(
            f"yield stress below target: {measured_yield_f:.1f} < {yield_target:.1f} Pa"
        )
    if syneresis_observed:
        fail_reasons.append("visible syneresis observed")
    if syneresis_pct_f is not None and syneresis_pct_f > syneresis_max:
        fail_reasons.append(
            f"syneresis above target: {syneresis_pct_f:.1f}% > {syneresis_max:.1f}%"
        )
    if ph_4h and ph_4h > 5.20:
        warnings.append("acidification is slow at hour 4; continue monitoring pH every 15 minutes")
    if ph_4h and ph_4h < 4.45:
        warnings.append("acidification is fast at hour 4; prepare to stop early when viscosity gate is reached")

    status = "FAIL" if fail_reasons else ("WATCH" if warnings else "PASS")
    return {
        "status": status,
        "traffic_light": "red" if status == "FAIL" else ("yellow" if status == "WATCH" else "green"),
        "targets": {
            "viscosity_min_Pa_s": viscosity_target,
            "yield_stress_min_Pa": yield_target,
            "syneresis_max_pct": syneresis_max,
        },
        "measured": {
            "ph_4h": ph_4h,
            "viscosity_Pa_s": measured_viscosity,
            "yield_stress_Pa": measured_yield_f,
            "syneresis_observed": syneresis_observed,
            "syneresis_pct": syneresis_pct_f,
        },
...
def recalibrate_from_feedback(candidate: Dict[str, Any], feedback: Dict[str, Any], lang: str = "zh") -> Dict[str, Any]:
    """Generate a temporary rescue instruction when the factory feedback fails QC.

    This is a rule-based prototype; the valuable product point is that the same
    feedback event is captured and can later train a stronger corrective model.
    """
    result = feedback.get("evaluation") if isinstance(feedback.get("evaluation"), dict) else evaluate_qc_feedback(candidate, feedback)
    if result.get("status") == "PASS":
        return {
            "status": "no_recalibration_needed",
            "instruction": {
                "zh": "本批次 QC 通过：不需要补救方案。按原 SOP 进入冷却与放行确认。",
                "en": "QC passed: no rescue action required. Continue with cooling and release confirmation.",
            },
            "operator_steps": [],
        }

    pwin = candidate.get("process_window", {}) or {}
    temp_display = ((pwin.get("display", {}) or {}).get(lang, {}) or {}).get(
        "fermentation_temperature_C",
        (pwin.get("fermentation_temperature_C", {}) or {}).get("display", "current window"),
    )
    max_rpm = int((pwin.get("maximum_shear", {}) or {}).get("post_fermentation_stir_rpm_max", 50))
    target_visc = _as_float(result.get("targets", {}).get("viscosity_min_Pa_s"), 1.5)
    measured_visc = _as_float(result.get("measured", {}).get("viscosity_Pa_s"), 0.0)
    ph_4h = _as_float(result.get("measured", {}).get("ph_4h"), 0.0)
    gap = max(0.0, target_visc - measured_visc)

    extension_min = 30
    if gap >= 0.30:
        extension_min = 45
    if ph_4h > 5.20:
        extension_min = max(extension_min, 60)

    rescue_rpm = min(30, max_rpm)
    if result.get("measured", {}).get("syneresis_observed"):
        rescue_rpm = min(25, max_rpm)

    instruction_zh = (
        f"系统已重新校准：请维持当前发酵温度窗口 {temp_display}，"
        f"将发酵时间延长 {extension_min} 分钟；后搅拌转速降至 {rescue_rpm} RPM；"
        f"每 15 分钟复测 pH 与粘度。达到 pH ≤ 4.6 且 η ≥ {target_visc:g} Pa·s 后立即停止并降温至 4°C。"
    )
    instruction_en = (
        f"System recalibrated: hold the current fermentation temperature window {temp_display}; "
        f"extend fermentation by {extension_min} minutes; reduce post-stir speed to {rescue_rpm} RPM; "
        f"recheck pH and viscosity every 15 minutes. Stop and cool to 4°C when pH <= 4.6 and η >= {target_visc:g} Pa·s."
    )

    return {
        "status": "temporary_rescue_instruction",
        "recalibration_id": f"RC-{candidate.get('candidate_id', 'C')}-v1",
        "root_cause_hypothesis": result.get("fail_reasons", []) + result.get("warnings", []),
        "new_setpoints": {
            "temperature_window": temp_display,
            "extend_fermentation_min": extension_min,
            "post_stir_rpm_max": rescue_rpm,
            "qc_recheck_every_min": 15,
        },
        "instruction": {"zh": instruction_zh, "en": instruction_en},
        "operator_steps": [
            "Hold current fermentation temperature window",
            f"Extend fermentation by {extension_min} min",
            f"Reduce post-stir speed to {rescue_rpm} RPM",
            "Recheck pH and viscosity every 15 min",
            "Stop when pH and viscosity gates are both met",
        ],
    }
...
def generate_candidates(
    ...
    for idx in range(k):
        ...
        candidate["simple_json"] = simplify_candidate(candidate, req.lang)
