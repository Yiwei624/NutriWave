...
def build_candidate_formulations(
    ...
    """
    Minimal candidate generator (v1):
    - Uses modular rules
    - Uses strain tags + goals to rank strains
    - Optionally biases sweetener/stabilizer by customer_profile (small, safe changes)
    """
    ...
    # strains (allow empty data by using placeholder)
    ...
    # generate variants by small grid
    random.seed(42)
        ...
        variants.append({
            ...
            "formula_100kg": [
                ...
                {"item_zh": f"{base.get('name_zh','')}蛋白", "item_en": f"{base.get('name_en','')} protein",
                 "kg": round(protein_pct, 2), "supplier": suppliers.get("proteins", {}).get(req.lang, "")},
                {"item_zh": "甜味剂", "item_en": "Sweetener",
                 "kg": round(s_lv, 2), "supplier": suppliers.get("sweeteners", {}).get(req.lang, "")},
                {"item_zh": "稳定剂/淀粉", "item_en": "Stabilizer/Starch",
                 "kg": round(st_lv, 2), "supplier": suppliers.get("stabilizers", {}).get(req.lang, "")},
                {"item_zh": "菌株", "item_en": "Strain",
                 "id": strain.get("id"), "name_zh": strain.get("name_zh"), "name_en": strain.get("name_en"),
                 "supplier": strain.get("uk_sup")}
            ],
            "process": {
...
def build_minidoe_plan(req: UserRequest) -> Dict[str, Any]:
    ...
    return {
        ...
        "suggested_figures": [
            "Candidate comparison table (syneresis/G'/τy)",
            "Sensory comparison (beany/sweet/smooth/overall)"
        ]
    }
